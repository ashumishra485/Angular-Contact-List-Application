import { Controller, Post, HttpStatus, HttpException, Body, Param, Put, Get, Delete } from '@nestjs/common';
...
export class UserController {
    ...
    @Post('addContact')
    @ApiResponse({ status: HttpStatus.CREATED, type: RegisterVm })
    ...
    async registerContact(@Body() register: RegisterVm): Promise<any> {
        ...
        return this._userService.map<RegisterVm>(newUser);
    }

    @Put('editContactList/:id')
    @ApiOkResponse({ description: 'Edit Calendar', type: RegisterVm })
    ...
    @Delete(':id')
    @ApiOkResponse({ description: 'Delete Upload', type: RegisterVm })
